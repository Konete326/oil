...
async function syncUpsert(Model, query, payload, targetId) {
  ...
  if (targetId && mongoose.isValidObjectId(targetId)) {
    return await Model.findByIdAndUpdate(targetId, cleaned, { new: true });
  }
  if (query) {
...
async function handleDelete(type, targetId) {
  ...
  if (type.startsWith("product")) await Product.findByIdAndDelete(targetId);
  else if (type.startsWith("category")) await Category.findByIdAndDelete(targetId);
  else if (type.startsWith("customer")) await Customer.findByIdAndDelete(targetId);
  else if (type.startsWith("expense")) await Expense.findByIdAndDelete(targetId);
  else if (type.startsWith("cash")) await CashTransaction.findByIdAndDelete(targetId);
  else if (type.startsWith("pos_sale")) await PosSale.findByIdAndDelete(targetId);
  else if (type.startsWith("supplier")) await Supplier.findByIdAndDelete(targetId);
  else if (type.startsWith("mill")) await Mill.findByIdAndDelete(targetId);
  else if (type.startsWith("challan")) await Challan.findByIdAndDelete(targetId);
  else if (type.startsWith("decanting")) await Decanting.findByIdAndDelete(targetId);
}
...
async function processSingleItem(item) {
  ...
  if (action === "delete" || type?.endsWith("_delete")) {
    await handleDelete(type, targetId);
    return;
  }
  ...
  } else if (type === "cash_entry") {
    await CashTransaction.create(cleaned);
  } else if (type === "expense_entry") {
    await Expense.create(cleaned);
  } else if (type === "customer_entry") {
    await syncUpsert(Customer, payload.name ? { name: payload.name } : null, payload, targetId);
  } else if (type === "product_entry") {
    await syncUpsert(Product, payload.name ? { name: payload.name } : null, payload, targetId);
  } else if (type === "category_entry") {
    await syncUpsert(Category, payload.name ? { name: payload.name } : null, payload, targetId);
  } else if (type === "supplier_entry") {
    await syncUpsert(Supplier, payload.name ? { name: payload.name } : null, payload, targetId);
  } else if (type === "mill_entry") {
    await syncUpsert(Mill, payload.name ? { name: payload.name } : null, payload, targetId);
  ...
  } else if (type === "decanting_entry") {
    await Decanting.create(cleaned);
...
export const syncBatchOfflineData = async (req, res) => {
  try {
    ...
    if (batchList.length === 0) {
      return res.status(200).json({ success: true, message: "No items to sync", syncedIds: [], processedCount: 0 });
    }
